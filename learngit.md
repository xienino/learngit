##### 写在前面
- 本文档是学习 [廖雪峰Git教程](http://t.cn/zQ6LFwE) 后所做的笔记
...
git stash  # 保存当前工作环境（包括工作区和暂存区）（藏匿）
...
git log --graph --pretty=oneline --abbrev-commit  #查看分支合并树状图
```
...
1. git pull 远程库
2. 解决冲突（若有），再push
git branch --set-upstream branch-name origin/branch-name #若no tracking infomation时
```
